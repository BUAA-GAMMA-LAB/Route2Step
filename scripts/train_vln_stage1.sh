#!/bin/bash
#
# train_vln_stage1.sh — Stage 1 M1 SFT via Slurm sbatch
#
# Usage:
#   bash scripts/train_vln_stage1.sh submit    # Submit Slurm job (default)
#   bash scripts/train_vln_stage1.sh status    # Show current user job status
#   bash scripts/train_vln_stage1.sh cancel    # Cancel jobs from job_id file
#   bash scripts/train_vln_stage1.sh logs      # Tail recent Slurm logs
...
# ---------- Training config ----------
MODEL="model_zoo/Qwen2.5-VL-3B-Instruct"
TRAIN_DATA="your_m1_dataset"
OUTPUT_DIR="path/to/m1_output"
...
cd "$(dirname "$0")/.." || exit 1
PROJECT_ROOT="$PWD"
LOG_DIR="$PROJECT_ROOT/slurm_logs/train_stage1"
...
echo "=========================================="
echo "Stage 1 M1 SFT: MODEL_PATH_PLACEHOLDER"
...
# ---- Submit ----
echo "============================================"
echo "  Stage 1 M1 SFT Slurm Launcher"
...
echo "  bash scripts/train_vln_stage1.sh status"
echo "  bash scripts/train_vln_stage1.sh logs"
